...
# create window
cv2.namedWindow('Vertical Timelapse',cv2.WINDOW_NORMAL)
...
cv2.setWindowProperty('Vertical Timelapse',cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
...
cv2.setMouseCallback('Vertical Timelapse',mouseClick)
...
while True:
    ...
    if start:
        lapsedFrame[ystart:,:]=frame[ystart:,:]
        frame[ystart:,:]=lapsedFrame[ystart:,:]
        cv2.line(lapsedFrame,(0,ystart+5),(width,ystart+5),(0,0,0),2)
        ystart+=1
    ...
    if start:
        cv2.imshow('Vertical Timelapse',lapsedFrame)
    else:
        cv2.imshow('Vertical Timelapse',frame)

    if cv2.waitKey(1) & 0xff==ord('q') or ystart>height+10:
        curr=''
        for i in range(6):
